...
class Model:
    ...
    # diagnostics
    def get_top_genes_in_slice(self,sl,blurred=False):
        fsc=self.F_scaled(blurred=blurred).numpy()[sl].reshape((-1,self.J))
        contribs=np.sum(fsc,axis=0)
        best=np.argsort(-contribs)
        return best

    def get_top_genes(self,blurred=True):
        return np.argsort(-self.F_scaled(blurred=blurred).numpy(),axis=-1)

    def reconstruction_sequence_in_slice(self,sl,gidxs=None):
        V=[self.ab_reconstruction()[sl].numpy()]

        f_slice=self.F_blurred[sl]
        G=self.frame_loadings()
        for gidx in gidxs:
            subrecon = tf.einsum('...,rc->...rc',f_slice[...,gidx],G[:,:,gidx])
            V.append(subrecon.numpy())
        return np.array(V)

    def cleanerplot(self,X,sl):
        X=tf.convert_to_tensor(X[sl])
        X=tf.reshape(X,[-1,self.N])

        fsc=tf.reshape(self.F_scaled(blurred=True)[sl],[-1,self.J])
        contribs = tf.reduce_sum(fsc,axis=0)

        fsc2=tf.reshape(self.F_blurred[sl],[-1,self.J])
        native_contribs=tf.reduce_sum(fsc2,axis=0)

        ab=tf.reshape(self.ab_reconstruction().numpy()[sl],[-1,self.R*self.C])

        G=self.frame_loadings()

        Xs=[tf.reduce_sum(X,axis=0).numpy()]
        Xs.append(Xs[-1]-tf.reduce_sum(ab,axis=0).numpy())
        for gidx in np.argsort(-contribs.numpy()):
            Xs.append(Xs[-1] - (native_contribs[gidx]*G[:,:,gidx]).numpy().ravel())

        return np.array(Xs),contribs
